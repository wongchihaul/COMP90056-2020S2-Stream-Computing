    ...
    public static void Question5(){
        ...
        for (int i = 0; i < d; i++) {
            ...
            for (int j = 0; j < numOfDistinct4 ; j++) {
                elements4.add((long) StdRandom.uniform(0,universe));
            }
            long bjkstCardinality4 = bjkst.compute(elements4);
            long hlogCardinality4 = hlog.compute(elements4);
...
//            if(i == 0) {System.out.println("Exact Distinct count: " +medianTrick(bsl, elements4,0.05));} else{break;}             //get the exact number distinct elememts
            ...
            for (int j = 0; j < numOfDistinct6 ; j++) {
                elements6.add((long) StdRandom.uniform(0, universe));
            }
            long bjkstCardinality6 = bjkst.compute(elements6);
            long hlogCardinality6 = hlog.compute(elements6);
...
//            int numOfDistinct = (int)Math.pow(10,i);                      //After running comparison with the distinct counts [10,100,1000,9955,95316,644380],
    ...
    public static void Question8(){
        ...
        for (int i = 10; i < 17; i++) {
            Hyperloglog hlog = new Hyperloglog(i);
